...
# Table derived from https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/MIME_types/Common_types
const mimes = """
...
var MIMETYPES {.threadvar.}: Table[string, string]

proc initMimeTable*() =
    if MIMETYPES.len == 0:
        MIMETYPES = initTable[string, string]()
        for line in mimes.split("\n"):
            let mime = line.split("\t")
            if mime.len == 3:
                MIMETYPES[strip(toUpper(mime[0]))] = mime[2]

# Return MIME type for a given extension
proc getMimeType*(extension: string): string =
  let ext = extension.strip().toUpperAscii()
