...
iterator terms(text: string): string =  # yield stemmed non-stop words in `text`
  for word in text.words:
    if word notin STOP: yield stem(word, stmEN)
...
if isMainModule:
    echo stem("friendly", stmEN)
    echo stem("freundlich", stmDE)
    echo stem("grösser", stmDE)

    echo stem("friendly", "EN")
    echo stem("freundlich", "DE")
    echo stem("grösser", "DE")